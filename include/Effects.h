...
public:
  ...
  CRGB currentColor = CRGB(0, 255, 0);  // Default color: green


 void NoiseVariablesSetup() {
  noise_x = random16(2);  // Zakres: 0 do 1
  noise_y = random16(2);  // Zakres: 0 do 1
  noise_z = random16(2);  // Zakres: 0 do 1
  noise_scale_x = random16(2) + 1;  // Zakres: 1 do 2
  noise_scale_y = random16(2) + 1;  // Zakres: 1 do 2
}
...
void FillNoise() {
  for (uint8_t i = 0; i < 80; i++) {
    ...
    for (uint8_t j = 0; j < 4; j++) {
      ...
      uint8_t newdata = scale8(olddata, 240) + scale8(data, 16); // Zmieniliśmy skalowanie.
      data = newdata;
...
void Pixel(int x, int y) {
  ...
  leds[XY(x * kMatrixWidth / 160, y * kMatrixHeight / 8)] = currentColor;
}
